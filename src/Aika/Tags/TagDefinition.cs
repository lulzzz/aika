...
namespace Aika.Tags {
    ...
    public abstract class TagDefinition {
        ...
        /// <summary>
        /// Gets the historian that owns the tag.
        /// </summary>
        public IHistorian Historian { get { return _historian; } }
        ...
        /// <summary>
        /// Gets the access security settings for the tag.
        /// </summary>
        public TagSecurity Security { get; private set; }
        ...
        /// <summary>
        /// Tests if the specified identity is authorized on the tag using any of the specified policy names.
        /// </summary>
        /// <param name="identity">The identity.</param>
        /// <param name="policyNames">The policy name.  Specify <see langword="null"/> to check if any policies on the tag authorize the user.</param>
        /// <returns>
        ...
        public bool IsAuthorized(ClaimsPrincipal identity, params string[] policyNames) {
            ...
            if (!String.IsNullOrWhiteSpace(Security.Owner)) {
                if (identity.HasClaim(ClaimTypes.NameIdentifier, Security.Owner) ||
                    identity.HasClaim(ClaimTypes.Name, Security.Owner)) {
            ...
            if (Security.Policies.Count == 0) {
                return false;
            }

            if (policyNames.Length == 0) {
                policyNames = Security.Policies.Keys.ToArray();
            }

            foreach (var name in policyNames) {
                if (!Security.Policies.TryGetValue(name, out var policy)) {
                    continue;
                }

                // Check if any rules allow the identity.
                var allow = policy.Allow?.Any(x => String.Equals(x.Value, "*")
                    ? true
                    : identity.HasClaim(x.ClaimType, x.Value)) ?? false;

                if (!allow) {
                    continue;
                }

                // Ensure that no rules deny the identity.
                var deny = policy.Deny?.Any(x => String.Equals(x.Value, "*")
                    ? true
                    : identity.HasClaim(x.ClaimType, x.Value)) ?? false;
